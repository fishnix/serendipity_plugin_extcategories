...
/*
    ...
    (extended from the built-in Categories plugin)
...
class serendipity_plugin_extcategories extends serendipity_plugin {
    var $title = EXTCATEGORIES;

    function introspect(&$propbag) {
        global $serendipity;

        $propbag->add('name',        EXTCATEGORIES);
        $propbag->add('description', EXTCATEGORY_PLUGIN_DESC);
        $propbag->add('stackable',     true);
        $propbag->add('author',        'Serendipity Team');
        $propbag->add('version',       '2.03');
        $propbag->add('configuration', array('title', 'desc', 'authorid', 'parent_base', 'hide_parent', 'image', 'sort_order', 'sort_method', 'allow_select', 'hide_parallel', 'show_count', 'smarty'));
    ...
    function introspect_config_item($name, &$propbag)
    {
        ...
        switch($name) {
            case 'title':
                ...
                $propbag->add('default',     EXTCATEGORIES);
                break;
                
            case 'desc':
                $propbag->add('type',        'string');
                $propbag->add('name',        DESC);
                $propbag->add('description', DESC_FOR_NUGGET);
                $propbag->add('default',     "Pick a Category");
                break;
    ...
    function generate_content(&$title) {
        ...
        if (!$smarty) {
            ...
            echo serendipity_smarty_fetch('CATEGORIES', 'plugin_extcategories.tpl');
        }
    }
}
?>
